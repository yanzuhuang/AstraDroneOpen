...
scripts/run_sh/fixed_orbit_inspection.sh \
...
`--attach` 会让当前终端进入 `fixed_orbit_inspection` 的 tmux 日志界面。若只想退出日志界面但保持仿真运行，先按 `Ctrl-b`，松开后再按 `d`。之后可用下面的命令重新进入：

```bash
tmux attach -t fixed_orbit_inspection
```
...
```bash
cd /home/yanzu/AstraDroneOpen
scripts/run_sh/fixed_orbit_inspection.sh --preview --waypoints 8 --attach
```

预览模式只启动路线节点和 RViz，不启动 PX4/Gazebo，也不会创建 MAVROS 控制发布者。关闭方式仍是 `scripts/run_sh/fixed_orbit_inspection.sh --stop`。
...
| `.../config/fixed_orbit_inspection.yaml` | 塔、航线、飞行、安全、Topic、服务和输出的唯一默认配置 |
| `.../launch/fixed_orbit_inspection.launch` | preview/control、点数、RViz 和报告文件入口 |
| `.../rviz/fixed_orbit_inspection.rviz` | 新手可直接使用的路线/目标/实际轨迹显示 |
| `.../test/tower_route_test.cpp` | 顺逆时针、严格半径、逐周期朝塔 yaw、检查点不停留和限幅单测 |
| `scripts/run_sh/fixed_orbit_inspection.sh` | 默认安全预览；显式 `--control` 才启动完整 PX4/Gazebo/FAST-LIO/任务链 |
| `offboard/CMakeLists.txt` | 导出并安装现有 `offboard/landing_profile.h`，供新包复用 |
| `fixed_orbit_inspection学习.md` | 本文 |
...
```bash
/home/yanzu/AstraDroneOpen/scripts/run_sh/fixed_orbit_inspection.sh --stop
```
...
scripts/run_sh/fixed_orbit_inspection.sh --control --mode hover --headless --attach

# 2. 单检查角计数模式，仍执行一整圈、返航和降落
scripts/run_sh/fixed_orbit_inspection.sh --control --mode mission --waypoints 1 --headless --attach

# 3. 四检查角计数模式，圆周几何不变
scripts/run_sh/fixed_orbit_inspection.sh --control --mode mission --waypoints 4 --headless --attach

# 4. 完整八检查点
scripts/run_sh/fixed_orbit_inspection.sh --control --mode mission --waypoints 8 --headless --attach
...
`/home/yanzu/AstraDroneOpen/AstraDrone_ros1_ws/src/MissionControl/astra_tower_mission/config/fixed_orbit_inspection.yaml`

除非表中另有说明，修改 YAML 后不需要重新编译，但必须安全停止并重新启动节点。角度单位是度，代码内部转换为弧度。

### | 参数名 | 所在文件 | 当前值 | 单位 | 作用 | 调大/调小的影响 | 建议范围 | 是否需要重新编译 |
|---|---|---:|---|---|---|---|---|
| `tower/name` | `fixed_orbit_inspection.yaml` | `radio_tower` | - | 日志和结果中的塔名 | 只改标签不会移动塔；必须与中心一起审查 | 与 world/测绘名称一致 | 否 |
| `tower/frame_id` | `fixed_orbit_inspection.yaml` | `map` | frame | 塔和路线坐标系 | 不能随意改；改后所有坐标/TF契约都变 | 当前只能 `map` | 否，但需架构审查 |
| `tower/center/x` | `fixed_orbit_inspection.yaml` | -10.0551 | m | worksite运行时塔中心 x | 平移整条路线 | 必须来自审计/测绘 | 否 |
| `tower/center/y` | `fixed_orbit_inspection.yaml` | 19.7104 | m | worksite运行时塔中心 y | 平移整条路线 | 必须来自审计/测绘 | 否 |
| `tower/collision_radius` | `fixed_orbit_inspection.yaml` | 6.41 | m | 塔碰撞保守包络，25 m切片尚待复核 | 调大更保守并可能拒绝半径；调小可能撞塔 | 25 m实测值或更保守值 | 否 |
| `mission/radius` | `fixed_orbit_inspection.yaml` | 14.0 | m | 无人机中心到塔中心半径 | 调大离塔远但路程长；调小更危险 | 当前值14，须结合障碍净空复核 | 否 |
| `mission/height` | `fixed_orbit_inspection.yaml` | 25.0 | m | 绝对 `map` 绕塔高度 | 调高可能遇到塔结构/地图上限；调低接近地面 | 当前默认25，换高度须重审 | 否 |
| `mission/waypoint_count` | `fixed_orbit_inspection.yaml` | 8 | 个 | 一圈的等角度检查点数 | 只影响检查事件和进度，不改变圆周几何或速度 | 正式任务8；1/4仅分级测试 | 否 |
| `mission/start_angle_deg` | `fixed_orbit_inspection.yaml` | -90.0 | deg | 第一点相对塔中心的方位 | 改变进场和闭环位置 | `[-180,180]` 易读，其他值会归一化 | 否 |
| `mission/direction` | `fixed_orbit_inspection.yaml` | `counter_clockwise` | - | 航点角度增减方向 | `clockwise` 顺时针；默认值逆时针 | 两者之一 | 否 |
| `mission/camera_yaw_offset_deg` | `fixed_orbit_inspection.yaml` | 0.0 | deg | 相机光轴相对机体+X偏角 | 正值会让机体 yaw 反向补偿同样角度 | 按标定，通常 `[-180,180]` | 否 |
| `mission/maximum_speed` | `fixed_orbit_inspection.yaml` | 0.30 | m/s | 水平/三维参考最大速度 | 调大更快但制动距离和误差增大；调小更稳但可能触发超时 | 初次 0.2–0.5 | 否 |
| `mission/maximum_acceleration` | `fixed_orbit_inspection.yaml` | 0.50 | m/s² | 参考加速度上限 | 调大响应快但更激烈；调小更平滑但更慢 | 初次 0.3–1.0 | 否 |
| `mission/maximum_yaw_rate_deg_s` | `fixed_orbit_inspection.yaml` | 30.0 | deg/s | 机头转速上限 | 调大转向快；调小可能 yaw 到达较慢 | 15–45 | 否 |
| `mission/position_tolerance` | `fixed_orbit_inspection.yaml` | 0.40 | m | 圆周入口及返航等位置到达容差 | 调大易进入下一阶段但精度差；调小可能超时 | 0.25–0.60 | 否 |
| `mission/yaw_tolerance_deg` | `fixed_orbit_inspection.yaml` | 12.0 | deg | 起飞、悬停、返航等到达条件的 yaw 容差 | 不控制检查点停留；绕塔 yaw 误差用于观测 | 5–15 | 否 |
| `mission/waypoint_timeout` | `fixed_orbit_inspection.yaml` | 240.0 | s | 到圆周入口的进场上限 | 调大更能容忍慢速；调小更快失败 | 120–300，须匹配距离/速度 | 否 |
| `mission/mission_timeout` | `fixed_orbit_inspection.yaml` | 600.0 | s | 进场和完整圆周总上限 | 调大允许更慢轨迹；调小可能中途失败 | 480–900 | 否 |
| `mission/overall_timeout` | `fixed_orbit_inspection.yaml` | 1200.0 | s | 从等待到结束的总上限 | 调大容忍启动/服务慢；调小更快终止 | 900–1800 | 否 |
| `mission/minimum_height` | `fixed_orbit_inspection.yaml` | 2.0 | m | 航线高度下限 | 调高排除低飞；调低减少地面保护 | 按 world/法规/定位确定 | 否 |
| `mission/maximum_height` | `fixed_orbit_inspection.yaml` | 27.0 | m | 航线高度上限 | 调高允许高飞；调低更保守 | 当前任务25，保留2 m余量 | 否 |
| `mission/minimum_safety_distance` | `fixed_orbit_inspection.yaml` | 2.0 | m | 航线半径减塔包络后的最小余量 | 调大更保守；调小风险增加 | 首测至少2 | 否 |
| `mission/maximum_home_distance` | `fixed_orbit_inspection.yaml` | 60.0 | m | home到任何航点的包络 | 调大允许更远任务；调小会拒绝当前路线 | 略高于几何最大距离 | 否 |
| `flight/takeoff_height` | `fixed_orbit_inspection.yaml` | 4.0 | m | home上方起飞和进场目标 | 调大起飞更高更久；调小需仍安全 | 当前默认4，须与任务高度协调 | 否 |
| `flight/takeoff_tolerance` | `fixed_orbit_inspection.yaml` | 0.25 | m | 起飞高度到达容差 | 同位置容差影响 | 0.2–0.5 | 否 |
| `flight/takeoff_hold_time` | `fixed_orbit_inspection.yaml` | 1.0 | s | 起飞到达连续保持 | 调大更稳、调小更快 | 1–3 | 否 |
| `flight/takeoff_timeout` | `fixed_orbit_inspection.yaml` | 60.0 | s | 起飞阶段超时 | 过小会误失败；过大延迟故障终止 | 45–120 | 否 |
| `flight/initial_hover_duration` | `fixed_orbit_inspection.yaml` | 3.0 | s | 入塔前稳定悬停 | 调大更稳但任务更久 | 2–10 | 否 |
| `flight/initial_hover_timeout` | `fixed_orbit_inspection.yaml` | 30.0 | s | 初始悬停阶段超时 | 应大于悬停时长并留误差收敛余量 | 20–60 | 否 |
| `flight/return_height` | `fixed_orbit_inspection.yaml` | 4.0 | m | 塔前下降后及home上方返航高度 | 与障碍和起飞高度协调 | 当前默认4，须审查返程净空 | 否 |
| `flight/return_hold_time` | `fixed_orbit_inspection.yaml` | 2.0 | s | home上方连续保持 | 调大更稳但更久 | 1–5 | 否 |
| `flight/return_timeout` | `fixed_orbit_inspection.yaml` | 240.0 | s | 返航超时 | 需匹配最远点和速度 | 180–360 | 否 |
| `flight/preland_hover_duration` | `fixed_orbit_inspection.yaml` | 3.0 | s | 下降前稳定时间 | 调大更稳、调小更快 | 2–10 | 否 |
| `landing/cruise_speed` | `fixed_orbit_inspection.yaml` | 0.40 | m/s | 高处OFFBOARD下降速度 | 调大降得快且风险增；调小更慢 | 0.2–0.5 | 否 |
| `landing/touchdown_speed` | `fixed_orbit_inspection.yaml` | 0.20 | m/s | 近地下降速度 | 调大接地更重；调小可能更易受噪声影响 | 0.10–0.25 | 否 |
| `landing/slowdown_height` | `fixed_orbit_inspection.yaml` | 1.20 | m | 开始减速高度 | 调大更早减速；调小更晚 | 0.8–2.0 | 否 |
| `landing/flare_height` | `fixed_orbit_inspection.yaml` | 0.30 | m | 近地最终速度目标高度 | 调大更保守；调小更贴地才变慢 | 0.2–0.5 | 否 |
| `landing/timeout` | `fixed_orbit_inspection.yaml` | 90.0 | s | 整个降落上限 | 调大容忍慢降；调小可能未接地就失败 | 60–150 | 否 |
| `safety/prestream_duration` | `fixed_orbit_inspection.yaml` | 3.0 | s | 请求OFFBOARD前预发送时间 | 调大等待更久更稳；不建议调小 | 2–5 | 否 |
| `safety/input_timeout` | `fixed_orbit_inspection.yaml` | 2.0 | s | MAVROS输入新鲜度 | 调大掩盖断流；调小对调度抖动敏感 | 1–3 | 否 |
| `safety/home_origin_tolerance` | `fixed_orbit_inspection.yaml` | 2.0 | m | 实际home与期望spawn差 | 调大可能掩盖坐标错位；调小可能因初始化漂移拒绝 | 1–3 | 否 |
...
```bash
scripts/run_sh/fixed_orbit_inspection.sh --preview --waypoints 12
```
...
# 只停止本脚本拥有的 fixed_orbit_inspection tmux 会话
scripts/run_sh/fixed_orbit_inspection.sh --stop
...
cd /home/yanzu/AstraDroneOpen
scripts/run_sh/fixed_orbit_inspection.sh --stop
scripts/run_sh/fixed_orbit_inspection.sh --control --mode mission --waypoints 8 --attach
```

也可用 `--restart` 让脚本只重启自己拥有的会话：

```bash
scripts/run_sh/fixed_orbit_inspection.sh --control --mode mission --waypoints 8 --restart --attach
```
...
```bash
roslaunch astra_tower_mission fixed_orbit_inspection.launch \
  enable_control:=true run_mode:=mission waypoint_count:=8 rviz:=true \
  report_file:=runtime_artifacts/fixed_orbit_inspection_manual_$(date +%Y%m%d_%H%M%S)/mission.csv
```
...
`--stop` 只向 `fixed_orbit_inspection` tmux 会话的窗口发送 `Ctrl-C`，等待 roslaunch 清理，再结束该会话。它不会用模糊名称强杀系统进程。
...
若脚本创建的会话还在，再执行一次 `fixed_orbit_inspection.sh --stop`。若发现其他会话或进程：
