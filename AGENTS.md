...
`training/astra_drone_env.py` 的正式入口是 `run_episode()`：保留 0.1 s ROS/sim-time nominal decision grid，但每个 Episode 最多只允许一个 open transition。上一 transition 只有在 stamped action/applied identity、trajectory provenance 与 causal post-hold Observation 全部闭合并按序提交 Replay 后，才允许生成下一 Actor request；Observation 未及时准备时记录 decision tick miss 并继续执行当前 applied `v_max`，不制造重叠 step。terminal 以 `ACTIVE -> TERMINATING` 原子闩锁阻止新 request，唯一 open transition exactly-once closure 后进入 `CLOSED`，coordinator 收到含零 outstanding/连续 Replay 审计的 ACK 后才可 reset。Gazebo/sensor/mapper/EGO/controller 与 simulation clock 仍连续异步运行。full-stack readiness 使用 mission/bridge/planner；training profile 使用 Hector coordinator/adapter identity。`AstraDroneEnv.reset()` 仍不直接拥有 reset，正式 training reset 由外部 coordinator 独占。
