...
session_name="fixed_orbit_inspection"
default_world="$repo_root/simulation/astra_gazebo_worlds/worksite.world"

usage() {
    cat <<'EOF'
用法：
  fixed_orbit_inspection.sh [--preview] [--waypoints N] [--attach]
  fixed_orbit_inspection.sh --control [--mode hover|mission] [--waypoints N]
                  [--headless] [--no-rviz] [--world FILE] [--attach]
                  [--report CSV_FILE] [--restart]
  fixed_orbit_inspection.sh --stop
...
选项：
  ...
  --report FILE   CSV 证据文件；默认写 runtime_artifacts/fixed_orbit_inspection_<时间戳>/
  --restart       只重启本脚本自己的 fixed_orbit_inspection tmux 会话
...
run_component() {
    ...
    case "$component" in
        preview)
            setup_ros_environment
            exec roslaunch astra_tower_mission fixed_orbit_inspection.launch \
...
fixed_orbit_processes_running() {
    local process_name
    for process_name in rosmaster rosout gzserver gzclient px4 mavros \
        fastlio_mapping tower_mission_node fixed_orbit_rviz; do
...
stop_owned_session() {
    ...
    for ((elapsed = 0; elapsed < 35; ++elapsed)); do
        if ! fixed_orbit_processes_running; then
            break
        fi
        sleep 1
    done
...
if [[ "$enable_control" == false ]]; then
    ...
    if [[ -z "$report_file" ]]; then
        evidence_dir="$repo_root/runtime_artifacts/fixed_orbit_inspection_${run_mode}_${waypoint_count}_$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$evidence_dir"
        report_file="$evidence_dir/mission.csv"
    fi
