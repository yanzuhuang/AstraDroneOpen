...
session_name="ego_waypoint_inspection"
default_world="$repo_root/simulation/astra_gazebo_worlds/worksite.world"

usage() {
    cat <<'EOF'
用法：
  ego_waypoint_inspection.sh [--scenario dry-run] [--attach]
  ego_waypoint_inspection.sh --control --scenario single|dual|tower
                [--waypoints N] [--gui] [--rviz] [--world FILE]
                [--report FILE] [--attach]
  ego_waypoint_inspection.sh --stop
...
run_component() {
    ...
    case "$component" in
        ...
        integration)
            ...
            exec roslaunch astra_tower_mission ego_waypoint_inspection.launch \
...
if [[ "$stop" == true ]]; then
    ...
    for _ in {1..35}; do
        if ! pgrep -x -f 'px4|gzserver|mavros|fastlio_mapping|ego_planner_node|traj_server|ego_mavros_bridge|ego_waypoint_mission_node' >/dev/null 2>&1; then
            break
        fi
        sleep 1
    done
...
if [[ -z "$report_file" ]]; then
    evidence_dir="$repo_root/runtime_artifacts/ego_waypoint_inspection_${scenario}_$(date +%Y%m%d_%H%M%S)"
