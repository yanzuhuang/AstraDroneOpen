...
while (($#)); do
  case "$1" in
    ...
    --duration)
      ...
      ;;
    --help)
      echo "three_uav_inspection.sh [--control] [--gui] [--rviz] [--duration SEC] [--results-dir DIR]"
      ...
      echo "Use --results-dir to select a timestamped runtime_artifacts/ directory."
      exit 0
      ;;
    *)
...
rosbag record --lz4 -O "$results_dir/three_uav_inspection.bag" "${topics[@]}" \
