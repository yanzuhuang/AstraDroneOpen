...
def latest_csv():
    candidates = list(
        Path("/tmp/astra_stage3_evidence").glob("control_*.csv")
    )
    if not candidates:
        raise FileNotFoundError(
            "No control CSV found under /tmp/astra_stage3_evidence"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "trc_picture",
    )
    ...
    output = args.output_dir.resolve() / (
        "stage3_actual_trajectory_{}.png".format(source.stem)
    )
    plot(
        samples,
        source,
    ...
    latest = output.parent / "latest_stage3_trajectory.png"
