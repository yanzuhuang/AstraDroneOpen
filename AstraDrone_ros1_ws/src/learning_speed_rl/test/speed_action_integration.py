...
class SpeedActionIntegrationTest(unittest.TestCase):
    ...
    def test_atomic_action_mirrors_existing_chain(self):
        ...
        self.assertEqual(action.version, "learning_speed_action_v1.1")
        self.assertEqual(action.source_mode, "fixed")
        self.assertEqual(action.episode_id, "__speed_adapter_fixed__")
        self.assertGreater(action.request_id, 0)
