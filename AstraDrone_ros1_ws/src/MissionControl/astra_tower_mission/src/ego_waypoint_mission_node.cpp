...
class EgoWaypointMissionNode {
 public:
  EgoWaypointMissionNode()
    ...
    timer_ = node_.createTimer(ros::Duration(1.0 / loop_rate_),
                               &EgoWaypointMissionNode::timerCallback, this);
    ROS_WARN("[EGO_WAYPOINT_TASK] scenario=%s control=%s initial_goals=%zu "
 ...
 private:
  void loadConfig() {
    ...
    private_node_.param<std::string>("report_file", report_file_,
                                     "runtime_artifacts/direct_ego_waypoint_inspection/mission.csv");
  ...
  bool buildNearestTowerReference(const ros::Time& now,
    ...
    ROS_WARN("[EGO_WAYPOINT_TASK] nearest tower=%s center=(%.3f, %.3f), "
  ...
  void setupRos() {
    odom_sub_ = node_.subscribe(odom_topic_, 20,
                                &EgoWaypointMissionNode::odomCallback, this);
    command_sub_ = node_.subscribe(command_topic_, 50,
                                   &EgoWaypointMissionNode::commandCallback,
                                   this);
    bridge_state_sub_ = node_.subscribe(
        bridge_state_topic_, 10,
        &EgoWaypointMissionNode::bridgeStateCallback, this);
  ...
  void openReport() {
    ...
    report_file_ = timestampedRuntimeArtifactFile(
        report_file_, "ego_waypoint_inspection");
    if (!ensureArtifactParentDirectory(report_file_)) {
      ROS_ERROR("[EGO_WAYPOINT_TASK] cannot create report directory for %s",
                report_file_.c_str());
      return;
    }
    report_.open(report_file_, std::ios::out | std::ios::trunc);
    if (!report_) {
      ROS_ERROR("[EGO_WAYPOINT_TASK] cannot open report %s", report_file_.c_str());
      return;
    }
  ...
  void transition(TaskState next, const std::string& reason) {
    ...
    ROS_WARN("[EGO_WAYPOINT_TASK] %s -> %s: %s", stateName(state_),
  ...
  void requestTracking(bool enabled) {
    ...
    if (!tracking_client_.call(service) || !service.response.success) {
      ROS_WARN("[EGO_WAYPOINT_TASK] tracking service rejected: %s",
  ...
  void finish(bool success, const std::string& reason) {
    ...
    ROS_WARN("[EGO_WAYPOINT_TASK] %s max_tracking_error=%.3f m report=%s",
        ...
        if (inputs_fresh && bridge_ready) {
          if (scenario_ == "tower" && !have_global_reference_) {
            ...
            if (!buildNearestTowerReference(now, &reason)) {
              ROS_WARN_THROTTLE(
                  1.0,
                  "[EGO_WAYPOINT_TASK] waiting to build nearest-tower reference: %s",
                  reason.c_str());
              break;
            }
          }
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "ego_waypoint_mission");
  try {
    astra_tower_mission::EgoWaypointMissionNode node;
    ros::spin();
  } catch (const std::exception& exception) {
    ROS_FATAL("[EGO_WAYPOINT_TASK] %s", exception.what());
    return 1;
  }
  return 0;
}
