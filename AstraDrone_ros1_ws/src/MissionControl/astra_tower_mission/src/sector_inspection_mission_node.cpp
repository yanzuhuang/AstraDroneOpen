/**
 * Sector-inspection EGO static-obstacle mission supervisor.
 *
...
class SectorInspectionMissionNode {
 public:
  SectorInspectionMissionNode() : node_(), private_node_("~") {
    loadConfig();
    ...
    timer_ = node_.createTimer(ros::Duration(1.0 / loop_rate_),
                               &SectorInspectionMissionNode::timerCallback, this);
    ROS_WARN("[SECTOR_INSPECTION_TASK] uav_id=%d control=%s takeoff_delay=%.1f "
 ...
 private:
  ...
  void logEntryGateSectorDefinitions() const {
    ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE map sectors: CCW from +X, "
    ...
    for (int user_sector = 1; user_sector <= 8; ++user_sector) {
      ...
      if (lower_deg > upper_deg) {
        ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE sector %d: %-13s center=%.1fdeg "
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] configured ENTRY_GATE sector=%d direction=%s "
  ...
  void loadConfig() {
    ...
    private_node_.param<std::string>("report_file", report_file_,
                                     "runtime_artifacts/direct_sector_inspection/mission.csv");
    ...
    if (low_altitude_mode_ && planner_map_size_x_ > 0.0 &&
      ...
      ROS_WARN("[SECTOR_INSPECTION_PREFLIGHT] EGO map envelope verified: %s",
               map_contract.c_str());
    }
    ...
    if (!validation || sector_count_ != 8 || sector_limit_ < 1 ||
      ...
      throw std::runtime_error(validation_reason_.empty()
                                   ? "invalid sector-inspection configuration"
                                   : validation_reason_);
    }
    if (!std::isfinite(inspection_height_) || !std::isfinite(recovery_height_max_) ||
      ...
      throw std::runtime_error("invalid sector-inspection height or recovery envelope");
    }
    if (offsets_.empty()) {
  ...
  void setupRos() {
    odom_sub_ = node_.subscribe(odom_topic_, 30,
                                &SectorInspectionMissionNode::odomCallback, this);
    command_sub_ = node_.subscribe(command_topic_, 50,
                                   &SectorInspectionMissionNode::commandCallback, this);
    cloud_sub_ = node_.subscribe(cloud_topic_, 5,
                                 &SectorInspectionMissionNode::cloudCallback, this);
    occupancy_sub_ = node_.subscribe(
        occupancy_topic_, 5, &SectorInspectionMissionNode::occupancyCallback, this);
    planner_status_sub_ = node_.subscribe(
        planner_status_topic_, 10, &SectorInspectionMissionNode::plannerStatusCallback,
        this);
    bridge_state_sub_ = node_.subscribe(
        bridge_state_topic_, 10, &SectorInspectionMissionNode::bridgeStateCallback,
        this);
    fcu_state_sub_ = node_.subscribe(
        mavros_state_topic_, 10,
        &SectorInspectionMissionNode::fcuStateCallback, this);
    extended_state_sub_ = node_.subscribe(
        mavros_extended_state_topic_, 10,
        &SectorInspectionMissionNode::extendedStateCallback, this);
    if (require_transition_permission_) {
      transition_permission_sub_ = node_.subscribe(
          transition_permission_topic_, 5,
          &SectorInspectionMissionNode::transitionPermissionCallback, this);
    }
    if (require_task_start_permission_) {
      task_start_permission_sub_ = node_.subscribe(
          task_start_permission_topic_, 5,
          &SectorInspectionMissionNode::taskStartPermissionCallback, this);
    }
    if (require_entry_permission_) {
      entry_permission_sub_ = node_.subscribe(
          entry_permission_topic_, 5,
          &SectorInspectionMissionNode::entryPermissionCallback, this);
      entry_corridor_selection_sub_ = node_.subscribe(
          entry_corridor_selection_topic_, 5,
          &SectorInspectionMissionNode::entryCorridorSelectionCallback, this);
    }
    ...
    report_file_ = timestampedRuntimeArtifactFile(
        report_file_, "sector_inspection");
    if (!ensureArtifactParentDirectory(report_file_)) {
      throw std::runtime_error("cannot create sector-inspection report directory");
    }
  ...
  void entryCorridorSelectionCallback(
    ...
    if (selected == entry_corridor_candidates_.end()) {
      ROS_WARN_THROTTLE(
          1.0,
          "[THREE_UAV_ENTRY] UAV%d ignored stale/unknown joint selection=%s",
    ...
    ROS_WARN(
        "[THREE_UAV_ENTRY] UAV%d joint selection latched id=%s angle=%.1fdeg "
  ...
  void landingPermissionCallback(const std_msgs::Bool::ConstPtr& msg) {
    if (msg->data && !landing_permission_) {
      ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d landing permission granted "
  ...
  void commandCallback(const quadrotor_msgs::PositionCommand::ConstPtr& msg) {
    ...
    if (awaiting_fresh_trajectory_ &&
      ...
      ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d EGO generated fresh trajectory "
  ...
  void bridgeStateCallback(const std_msgs::String::ConstPtr& msg) {
    if (!have_bridge_state_ || bridge_state_ != msg->data) {
      ...
      if (msg->data == "HOME_HOVER") {
        ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d home hover reached "
  ...
  bool configureAscentChannel(int channel_index, bool reset_history) {
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] fixed vertical ascent locked at XY=(%.2f, %.2f); "
  ...
  bool selectAlternateAscentChannel() {
    ROS_ERROR("[SECTOR_INSPECTION_TASK] alternate ascent channels are disabled; "
  ...
  bool buildEntryCorridorCandidates(const ros::Time& now) {
    ...
    if (orbit_candidates.empty()) {
      ROS_WARN("[THREE_UAV_ENTRY] UAV%d no accepted ordinary-sector orbit "
    ...
    ROS_WARN(
        "[THREE_UAV_ENTRY] UAV%d generation=%u searched=%zu safe=%zu "
  ...
  void invalidateEntryCorridor(const ros::Time& now,
    ...
    ROS_ERROR("[THREE_UAV_ENTRY] UAV%d selected corridor invalidated before "
  ...
  bool climbTargetReady(const CandidatePoint& target, const ros::Time& now,
    ...
    ROS_INFO_THROTTLE(
        1.0,
        "[SECTOR_INSPECTION_TASK] climb coverage target=%s endpoint_unknown=%.3f "
    ...
    if (current_coverage_unknown_ratio_ >
      ...
      ROS_WARN_THROTTLE(
          2.0,
          "[SECTOR_INSPECTION_TASK] MAP_UNKNOWN on vertical channel is distinct from "
  ...
  bool lockLayerGate(int layer_index, const ros::Time& now) {
    ...
    for (auto& candidate : entry_gate_candidates_) {
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE candidate layer=%d id=%s "
      ...
      if (!candidate.accepted) {
        ROS_DEBUG("[SECTOR_INSPECTION_TASK] layer gate candidate rejected: layer=%d "
    ...
    if (entry_gate_index_ < 0) {
      ...
      ROS_ERROR_THROTTLE(
          1.0,
          "[SECTOR_INSPECTION_TASK] %s: direction=%s layer=%d candidates=%zu; "
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE selected: layer=%d %s "
  ...
  bool startEntryGateTransit() {
    ...
    if (low_altitude_mode_) {
      ...
      transition(MissionState::kEntryGateTransit,
                 ...
                 "to EGO");
      ROS_WARN("[SECTOR_INSPECTION_TASK] LOW_ALTITUDE corridor UAV%d PRE_ENTRY="
    ...
    if (prefer_safe_overflight_ && mappedCorridorSafe(current, gate)) {
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] fresh map confirms a clear high corridor; "
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] high straight corridor is not fully clear; "
  ...
  bool relocateEntryGateWithinConfiguredSector(
    ...
    if (entry_gate_locked_) {
      ...
      ROS_ERROR("[SECTOR_INSPECTION_TASK] ENTRY_GATE candidate %s rejected as "
    ...
    if (entry_gate_relocations_ <= max_entry_gate_relocations_ &&
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE relocated inside configured sector %d; "
    ...
    ROS_ERROR("[SECTOR_INSPECTION_TASK] %s after %d rejected candidates; "
  ...
  bool buildInspectionLayer(int layer_index) {
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] inspection layer %d prepared: height=%.2f; "
  ...
  void publishFixedMissionRoute(const ros::Time& now) {
    ...
    ROS_WARN("[SECTOR_INSPECTION_GOAL] fixed mission route published: "
  ...
  bool lockDirectionalInitialSector(const ros::Time& now,
    ...
    const char* direction_name =
        route_.direction == OrbitDirection::kCounterClockwise
            ? "counter_clockwise"
            : "clockwise";
    ROS_WARN("[SECTOR_INSPECTION_TASK] FIRST_WAYPOINT %s layer=%d "
    ...
    for (std::size_t order_index = 0; order_index < sectors_.size();
      ...
      const double candidate_angle = candidate == nullptr
                                         ...
                                         : std::atan2(candidate->y - route_.center_y,
                                                      candidate->x - route_.center_x);
      ROS_WARN("[SECTOR_INSPECTION_TASK] FIRST_WAYPOINT candidate waypoint=%d "
    ...
    if (entry_corridor_locked_ &&
      ...
      if (selected_order_index < 0) {
        ROS_ERROR("[THREE_UAV_ENTRY] UAV%d jointly locked orbit candidate %s "
    ...
    if (selected_order_index < 0 || selected_candidate_index < 0) {
      ...
      ROS_ERROR("[SECTOR_INSPECTION_TASK] %s layer=%d; reverse/opposite-side "
    ...
    if (entry_corridor_locked_ &&
      ...
      if (std::isfinite(staging_radius) &&
        ...
        ROS_WARN(
            "[THREE_UAV_ENTRY] UAV%d applying jointly selected staging "
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] FIRST_WAYPOINT selected waypoint=%d "
             ...
             std::sqrt((current.x - active_target_.x) *
                       ...
                       (current.z - active_target_.z) *
                           (current.z - active_target_.z)));
    ROS_WARN("[SECTOR_INSPECTION_TASK] FIRST_WAYPOINT reordered execution sequence "
             "layer=%d direction=%s order=%s",
             current_layer_, direction_name, execution_order.str().c_str());
    ROS_WARN("[SECTOR_INSPECTION_TASK] first waypoint remains pending actual EGO "
  ...
  bool evaluateSector(const ros::Time& now) {
    ...
    if (closing_at_layer_anchor) {
      ...
      current_target_plan_attempt_ = 0;
      ROS_WARN("[SECTOR_INSPECTION_TASK] closing layer=%d lap=%d at start anchor "
    ...
    if (selected < 0) {
      ...
      for (const auto& candidate : sector.candidates) {
        ...
        const double radius = std::hypot(candidate.x - route_.center_x,
                                         candidate.y - route_.center_y);
        ROS_ERROR("[SECTOR_INSPECTION_CANDIDATE_EXHAUSTED] uav=%d sector=%d id=%s "
      ...
      ROS_ERROR("[SECTOR_INSPECTION_CANDIDATE_EXHAUSTED] uav=%d sector=%d total=%zu "
      ...
      if (low_altitude_mode_ &&
        ...
        current_target_plan_attempt_ = 0;
        ROS_WARN("[SECTOR_INSPECTION_TASK] uav=%d sector=%d known-obstacle "
    ...
    ROS_WARN("[THREE_UAV_ORBIT_TIER] uav=%d visit=%zu sector=%d "
  ...
  bool requestTracking(bool enabled) {
    ...
    if (!tracking_client_.call(service) || !service.response.success) {
      ROS_ERROR("[SECTOR_INSPECTION_TASK] tracking %s rejected: %s", enabled ? "resume" : "hold",
  ...
  bool requestHold(const std::string& reason) {
    ...
    ROS_ERROR("[SECTOR_INSPECTION_DIAG] uav=%d HOLD requested mission_state=%s "
    ...
    if (enable_control_) {
      ...
      if (!cancelled) ROS_WARN("[SECTOR_INSPECTION_TASK] cancel trajectory unavailable: %s",
  ...
  bool requestResume() {
    ...
    if (!resume_client_.call(service) || !service.response.success) {
      ROS_WARN("[SECTOR_INSPECTION_TASK] EGO resume pending: %s",
               service.response.message.c_str());
  ...
  bool buildRecovery(const ros::Time& now) {
    ...
    if (!configured_direction_local) {
      ROS_ERROR("[THREE_UAV_ORBIT_ORDER] uav=%d sector=%d configured %s "
    ...
    if (orbit_tracking_initialized_) {
      ...
      for (const auto& target : recovery_sequence) {
        ...
        if (!orbitTargetAtOrAhead(angle, orbit_start_angle_,
          ...
          ROS_ERROR("[THREE_UAV_ORBIT_ORDER] uav=%d sector=%d recovery target "
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] sector-local recovery selected %s at z=%.2f; "
  ...
  bool prepareNextLayerAnchor(int next_layer, const ros::Time& now,
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] next layer start selected: layer=%d "
  ...
  bool startLayerTransition(const ros::Time& now, std::string* reason) {
    ...
    const bool same_xy =
        ...
        layer_transition_same_xy_tolerance_;
    ROS_WARN("[SECTOR_INSPECTION_TASK] layer transition accepted: layer=%d->%d "
  ...
  bool startExitGate(const ros::Time& now, const std::string& reason) {
    ...
    have_sent_goal_ = false;
    arrival_since_ = ros::Time(0);
    ROS_WARN("[SECTOR_INSPECTION_TASK] final return selected: layer=%d height=%.2f "
  ...
  bool startNormalReturn(const std::string& reason,
    ...
    if (include_exit_gate) {
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] final-layer EXIT_GATE selected: layer=%d "
    ...
    ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d return state entered "
  ...
  bool normalReturnGoalSafe(const CandidatePoint& target,
    ...
    if (!straight_clear) {
      ROS_WARN_THROTTLE(
          1.0,
          "[SECTOR_INSPECTION_TASK] normal-return straight corridor blocked; "
  ...
  bool requestAutoLandAtHome(const ros::Time& now) {
    ...
    if (!land_client_.call(land) || !land.response.success) {
      ROS_ERROR("[SECTOR_INSPECTION_TASK] AUTO.LAND rejected at home hover: %s",
  ...
  bool buildReturnEgress() {
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] safe return egress selected: %s, goals=%zu, "
  ...
  void lockFormalTargetCoordinates(const std::string& mission_target_id,
    ...
    ROS_ERROR("[SECTOR_INSPECTION_GOAL] blocked coordinate mutation for formal target "
  ...
  void noteEgoGoalPublication(const CandidatePoint& target,
    ...
    ROS_WARN("[SECTOR_INSPECTION_GOAL] publication=FORMAL_MISSION_TARGET "
  ...
  void publishGoal(const CandidatePoint& target, GoalKind kind) {
    ...
    const CandidatePoint* entry_exit_gate =
        current_layer_ >= 0 &&
            ...
            : nullptr;
    ROS_WARN("[SECTOR_INSPECTION_TASK] goal phase=%s layer=%d sector=%d id=%s "
  ...
  void transition(MissionState next, const std::string& reason) {
    ...
    ROS_WARN("[SECTOR_INSPECTION_TASK] uav=%d %s -> %s: %s; layer=%d height=%.2f "
  ...
  void timerCallback(const ros::TimerEvent&) {
    ...
    if (state_ == MissionState::kWaitInputs) {
      ...
      if (have_odom_ && fresh(now, odom_received_, input_timeout_) &&
        ...
        ROS_INFO_THROTTLE(
            1.0,
            "[SECTOR_INSPECTION_DIAG] uav=%d HOVER_READY waiting unified task-start "
      ...
      if (!mapFresh(now)) {
        ROS_INFO_THROTTLE(
            1.0,
            "[THREE_UAV_ENTRY] UAV%d waiting for fresh map before corridor "
      ...
      if (entry_gate_transit_index_ >= entry_gate_transit_goals_.size()) {
        ...
        if (!mapFresh(now)) {
          ...
          if (!endpoint_blockage.empty()) {
            ...
            if (!static_risk.empty()) {
              ...
              ROS_WARN("[SECTOR_INSPECTION_TASK] ENTRY_GATE_STATIC_CORRIDOR_RISK at %s "
        ...
        if (plannerFailure(now, &failure)) {
          if (low_altitude_mode_) {
            ROS_WARN_THROTTLE(
                1.0,
                "[SECTOR_INSPECTION_TASK] EGO internal ENTRY_GATE replan status=%s; "
      ...
      if (!entrySystemsHealthy(now, &entry_reason)) {
        ...
        if (!requestResume()) {
          ...
          ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d MOVE_TO_ORBIT_STAGING accepted "
      ...
      if (!entrySystemsHealthy(now, &entry_reason)) {
        ...
        if (!requestResume()) {
          ...
          if (visit_cursor_ >= lap_visit_sequence_.size()) {
            ...
            ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d individual ORBIT_RELEASE "
      ...
      if (!mapFresh(now)) {
        ...
        if (lockDirectionalInitialSector(now, true)) {
          ROS_WARN("[SECTOR_INSPECTION_TASK] first standard waypoint "
      ...
      if (waypoint_reached) {
        ...
        if (low_altitude_mode_ ||
          ...
          if (sector_detour_active_ &&
            ...
            ROS_WARN("[SECTOR_INSPECTION_TASK] uav=%d sector=%d static detour reached; "
          ...
          if (initial_waypoint_pending_ &&
            ...
            ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d ORBIT_STAGING_READY "
          ...
          if (visit_cursor_ >= lap_visit_sequence_.size()) {
            ...
            if (next_layer_available) {
              ...
              ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d exit permission requested "
      ...
      if (layer_transition_index_ >= layer_transition_goals_.size()) {
        ...
        if (!layerTransitionSegmentSafe(target, now, &safety_reason)) {
          ...
          const int next_layer =
              layer_visit_sequence_[current_layer_visit_index_ + 1U];
          ROS_WARN("[SECTOR_INSPECTION_TASK] LAYER_TRANSITION layer=%d->%d "
      ...
      if (exit_permission_ready) {
        ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d exit permission granted "
      ...
      if (!have_sent_goal_) {
        ...
        if (low_altitude_mode_ ||
          ...
          if (low_altitude_mode_) {
            ...
            ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d exit gate reached layer=%d "
      ...
      if (now - state_entered_ >= ros::Duration(failure_hold_duration_)) {
        if (coordination_hold_pending_) {
          ...
          if (!coordination_restored) {
            ROS_ERROR_THROTTLE(
                1.0,
                "[SECTOR_INSPECTION_TASK] swarm safety/phase permission remains "
            ...
            if (requestResume() && entry_gate_locked_ &&
              ...
              ROS_WARN(
                  "[SECTOR_INSPECTION_DIAG] uav=%d ENTRY safety HOLD recovered "
          ...
          if (entry_gate_recheck_time_.isZero() ||
            ...
            if (low_altitude_mode_ && require_entry_permission_) {
              ...
              if (buildEntryCorridorCandidates(now)) {
                ...
                ROS_ERROR_THROTTLE(
                    1.0,
                    "[THREE_UAV_ENTRY] UAV%d no dynamic full-corridor "
              ...
              entry_gate_failure_pending_ = false;
              entry_gate_relocation_pending_ = false;
              ROS_WARN("[SECTOR_INSPECTION_TASK] fresh map exposed a safe reachable "
                       "ENTRY_GATE candidate inside sector %d; leaving HOLD",
                       entry_sector_user_);
            } else {
              ROS_ERROR_THROTTLE(
                  1.0,
                  "[SECTOR_INSPECTION_TASK] %s; no other sector will be searched, "
          ...
          if (normal_return_retries_ <= max_normal_return_retries_ &&
            ...
            if (next_index < normal_return_goals_.size()) {
              ROS_WARN(
                  "[SECTOR_INSPECTION_DIAG] uav=%d blocked reverse-ingress endpoint "
      ...
      if (normal_return_index_ >= normal_return_goals_.size()) {
        ...
        if (!normalReturnGoalSafe(normal_return_goals_[normal_return_index_],
          ...
          ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d return goal published "
      ...
      if (bridge_state_ == "HOME_HOVER") {
        if (!landing_permission_ready) {
          ROS_INFO_THROTTLE(
              2.0,
              "[SECTOR_INSPECTION_TASK] HOME_HOVER: waiting for swarm landing permission");
  ...
  void requestReturnOrLand(const std::string& reason) {
    ...
    if (have_odom_ && have_home_position_ &&
      ...
      ROS_WARN("[SECTOR_INSPECTION_TASK] RETURN_EGRESS unavailable; attempting bounded "
  ...
  void requestBridgeReturnOrLand(const std::string& reason) {
    ...
    if (return_client_.call(service) && service.response.success) {
      ...
      awaiting_fresh_trajectory_ = true;
      ROS_WARN("[SECTOR_INSPECTION_DIAG] uav=%d bridge accepted goal service=return_home "
      ...
      ROS_ERROR("[SECTOR_INSPECTION_TASK] return rejected: %s", service.response.message.c_str());
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "sector_inspection_mission");
  try {
    astra_tower_mission::SectorInspectionMissionNode node;
    ros::spin();
  } catch (const std::exception& exception) {
    ROS_FATAL("[SECTOR_INSPECTION_TASK] %s", exception.what());
    return 1;
  }
  return 0;
}
