...
scripts/run_sh/sector_inspection.sh --control --sector-limit 8 --cycles 1 \
  --bag runtime_artifacts/sector_inspection_control_$(date +%Y%m%d_%H%M%S)/sector_inspection.bag
```
...
着色的三维实际轨迹和高度曲线，保存到本轮 `runtime_artifacts/` 时间戳目录。时间戳文件保留
每轮结果，`latest_inspection_trajectory.png` 始终指向最新生成结果。也可对任意
历史 CSV 手动重画：

```bash
python3 scripts/tool/plot_inspection_trajectory.py \
  --csv runtime_artifacts/sector_inspection_control_20260726_003511/control.csv
```
...
rostest astra_tower_mission sector_inspection_no_control.test
```
...
2026-07-26 的历史 CSV 曾按当时流程生成轨迹图；旧图目录已删除。后续重绘图统一
写入本轮 `runtime_artifacts/sector_inspection_<时间戳>/`，并更新
`latest_inspection_trajectory.png`。
...
`astra_tower_mission/config/sector_inspection.yaml`。入口不再搜索其他侧或低空等待点；
...
```bash
cd /home/yanzu/AstraDroneOpen
scripts/run_sh/sector_inspection.sh --sector-limit 8 \
  --gui --rviz --attach
```
...
```bash
tmux attach -t sector_inspection
```
...
```bash
bash scripts/run_sh/sector_inspection_record_bag.sh
```
...
`--rviz` 打开 RViz，`--attach` 让当前终端附着到 `sector_inspection` tmux 会话并显示
任务日志。命令中不带 `--bag`，因此不会启动 rosbag recorder。

`--report FILE` 是任务进度 CSV，不是 rosbag；其中记录状态转换、目标和时间线。
如果省略 `--report`，脚本会自动把 CSV 写入独立的
`runtime_artifacts/stage3_<mode>_<时间戳>/` 目录。
...
scripts/run_sh/sector_inspection.sh --control --sector-limit 8 \
  --bag runtime_artifacts/sector_inspection_control_<时间戳>/sector_inspection.bag \
  --gui --rviz --attach
```
...
为 `sector_inspection.sh --control --sector-limit 1`；没有执行 8 扇区控制飞行。
...
当前新增参数和语义见第 0 节以及 `sector_inspection.yaml`。

阶段三配置：`AstraDrone_ros1_ws/src/MissionControl/astra_tower_mission/config/sector_inspection.yaml`
...
* `astra_tower_mission/include/.../inspection_candidate_planner.h`、
  `src/inspection_candidate_planner.cpp`：本轮增加名义点优先和严格字典序、覆盖硬约束、
  直线走廊软风险、固定 XY 分段和显式闭环多圈；历史上还包括三维滚动
  子目标、吊机 yaw 定向三维包围盒、最近已接受扇区选择、同扇区高空候选、
  安全 RETURN_EGRESS 生成、home 落地点约束和已有候选/恢复逻辑；
* `src/sector_inspection_mission_node.cpp`：本轮增加 provisional gate、STAGING_POINT、
...
* `config/sector_inspection.yaml`、`test/inspection_candidate_planner_test.cpp`、
  `test/sector_inspection_no_control.{test,py}`；
...
* `scripts/run_sh/sector_inspection.sh`、`sector_inspection_record_bag.sh`：修正 odom 等待，
  支持 `--bag` 在任务启动前集成录制，补齐控制出口和诊断 topic，并拒绝覆盖
  已有 bag；停止时先等待 recorder 完成索引；
* `scripts/tool/analyze_inspection_bag.py`：bag 必需 topic、状态、扇区唯一性、规划
...
```text
相关包与 sector_inspection_mission_node 构建     通过
catkin_test_results                   146 tests
errors / failures                     0 / 0
sector_inspection.sh / recorder bash -n       通过
analyze_inspection_bag.py py_compile       通过
```
...
source /opt/ros/noetic/setup.bash
rosbag info runtime_artifacts/sector_inspection_dry_run_<时间戳>/sector_inspection.bag
...
cd /home/yanzu/AstraDroneOpen
python3 scripts/tool/analyze_inspection_bag.py \
  /home/yanzu/bag/astra_stage3_eight_sector_2026-07-23.bag \
  --occupancy-stride 10 \
  --output \
